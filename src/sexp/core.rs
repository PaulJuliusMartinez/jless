...
impl DocCore {
    ...
    // Returns the first data node (list / atom / unit) inside a list (even if it's
    // a commented out node). Returns `None` if the given node is not the start of a list.
    pub fn first_data_elem(&self, node_index: NodeIndex) -> Option<NodeIndex> {
